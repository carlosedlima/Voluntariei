...
class MyDataViewModel @Inject constructor(
    ...
    fun updateUser(name:String,email:String,pass:String,document:String)=viewModelScope.launch {
        isFormValid =true
        ...
        if (isFormValid){
            try {
                val user = userUseCase.updateUser(User(null,name,email,pass,document))
    ...
    fun upsdateOng(name:String,email:String,pass:String)=viewModelScope.launch {
        isFormValid =true
        ...
        if (isFormValid){
            try {
                val ong = ongUseCase.updateOng(ONG(1,name,email,pass))
